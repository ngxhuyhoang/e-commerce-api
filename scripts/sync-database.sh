...
docker exec e-commerce-database sh -c 'exec mysqldump --databases e_commerce_database -uhoang -p12345678 -h66.42.53.107 --verbose' > e_commerce_database_backup.sql
...
docker exec -i e-commerce-database sh -c 'exec mysql -uroot -p12345678 --database e_commerce_database --verbose' < e_commerce_database_backup.sql
